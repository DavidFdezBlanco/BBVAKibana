...
data "aws_eip" "static_back_ip_pre_prod" {
  filter {
    name   = "tag:machine_type"
    values = ["database_machine"]
  }
  filter {
    name   = "tag:env"
    values = [var.env]
...
resource "aws_instance" "backend_machine" {
  ami           = var.vm_ami
...
resource "aws_eip_association" "eip_assoc_swarm_leader" {
  instance_id   = "${aws_instance.backend_machine.id}"
  allocation_id = "${data.aws_eip.static_back_ip_pre_prod.id}"
}
