...
function generateRating(id, punctuation, comment, date, lat, lng, country, cluster_id, subcluster_id) {
    let rating = {
        id: parseInt(id), punctuation: parseFloat(punctuation), comment, date: new Date(date), lat: parseFloat(lat), lng: parseFloat(lng), country,
    ...
    .on('data', function (row) {
        ...
        switch (parseInt(typeData)) {
            ...
            case 1: //subcluster
                ...
                break;
            case 2: //rating
                const rating = generateRating(element[1], element[2], element[3], element[4], element[5], element[6], element[7], element[8], element[9]);
                ratings.push(rating);
                break;
            default:
                break;
